...
def _blend_scalars(
    a_list: list[float], b_list: list[float], alpha: float
) -> list[float]:
...
def MBPLS_EM(
    ...
    min_var_e: float | None = None,
    min_var_eps: float | None = None,
    verbose: bool = False,
    ...
    for it in range(max_iter):
        ...
        P_new, _ = update_P(
            data=data,
            W=(
                W_new if damping == 1.0 else params["W"]
            ),  # small subtlety for consistency
            mu_U_list=E["mu_U"],
            S_uu_list=E["Suu"],
    ...
    history = dict(
        loglik=loglik_hist, loglik_blocks=loglik_blocks_hist, iters=len(loglik_hist)
    )
